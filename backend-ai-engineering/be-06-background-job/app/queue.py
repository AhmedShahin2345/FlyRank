...
from app.schemas import (
    JobStatus, JobPriority, JobCreate, JobDetail, JobListResponse,
    EnrichJobInput, EnrichJobOutput, HealthResponse, WorkerStats
)
...
class JobQueue:
    def __init__(self, redis_client: Redis):
        ...
        self.processing_key = "jobs:processing"
        self.job_prefix = "job:"
        self.idempotency_prefix = "idempotency:"
        self.dlq_key = "jobs:dead_letter"
        self.stats_key = "worker:stats"
        self.lock_prefix = "lock:job:"

    async def _get_job_key(self, job_id: str) -> str:
        return f"{self.job_prefix}{job_id}"

    async def _get_lock_key(self, job_id: str) -> str:
        return f"{self.lock_prefix}{job_id}"

    async def create_job(self, job_data: JobCreate) -> JobDetail:
        """Create a new job with optional idempotency key."""
        ...
        # Using positive values so ZPOPMAX returns highest priority first
        await self.redis.zadd(
            self.queue_key,
            {job_id: job_data.priority.value}
        )
    ...
    async def get_job(self, job_id: str) -> Optional[JobDetail]:
        ...
        if data:
            return JobDetail.model_validate_json(data)
        return None

    async def update_job(self, job: JobDetail) -> None:
    ...
    async def claim_next_job(self, worker_id: str) -> Optional[JobDetail]:
        """Atomically claim the next highest-priority job."""
        ...
        if result:
            ...
            job = JobDetail.model_validate(job_data)
    ...
    async def complete_job(self, job_id: str, worker_id: str, output: EnrichJobOutput) -> bool:
    ...
    async def fail_job(self, job_id: str, worker_id: str, error: str) -> bool:
        ...
        if job.retries > settings.MAX_RETRIES:
            ...
            # Alert if configured
            await self._alert_dead_letter(job)
        else:
            job.status = JobStatus.QUEUED
            job.started_at = None
            # Re-queue with same priority
    ...
    async def _alert_dead_letter(self, job: JobDetail) -> None:
        """Log dead letter for alerting (webhook if configured)."""
        alert_data = {
            "type": "dead_letter",
            "job_id": job.job_id,
            "error": job.error,
        ...
        await self.redis.lpush("alerts:dead_letter", json.dumps(alert_data))
        await self.redis.ltrim("alerts:dead_letter", 0, 99)
        
        # TODO: If ALERT_WEBHOOK_URL is configured, POST there

    async def _trigger_callback(self, job: JobDetail) -> None:
        """Trigger callback webhook (best effort)."""
        if not job.callback_url:
            return
        # In production, use a proper HTTP client with retries
        # For now, just log
        pass

    async def list_jobs(self, limit: int = 50, offset: int = 0, status: Optional[JobStatus] = None) -> JobListResponse:
        ...
        while count < limit + offset:
            ...
            for key in keys:
                ...
                if data:
                    job = JobDetail.model_validate_json(data)
    ...
    async def get_dead_letter_jobs(self, limit: int = 50) -> List[JobDetail]:
        """Get jobs from dead letter queue."""
        jobs = []
        for i in range(min(limit, await self.redis.llen(self.dlq_key))):
            data = await self.redis.lindex(self.dlq_key, i)
            if data:
                jobs.append(JobDetail.model_validate_json(data))
        return jobs

    async def retry_dead_letter_job(self, job_id: str) -> Optional[JobDetail]:
